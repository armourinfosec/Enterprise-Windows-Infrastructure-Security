...
Windows Server's **Remote Access** server role (RRAS — Routing and Remote Access Service) provides the platform for VPN remote access, dial-up/site-to-site routing, and (paired with **Network Policy Server**) centralized RADIUS authentication for VPN, RDS, and 802.1X connections. This note covers the RRAS role, the four classic VPN tunneling protocols, VPN server/client configuration, RDP/RDS remote access, NPS, and the hardening steps needed to run remote access safely on the open internet.
...
- Install and configure the RRAS Remote Access role for VPN and routing
- Choose between the four VPN tunnel protocols (PPTP, L2TP/IPsec, SSTP, IKEv2) and configure server and client securely
- Centralize authentication with NPS/RADIUS and harden RDP/VPN against internet-facing attack
...
| [VPN-Types](VPN-Types.md) | VPN tunnel protocol types |
| [RRAS](RRAS.md) | Routing and Remote Access Service |
| [SSTP](SSTP.md) | Secure Socket Tunneling Protocol |
| [Remote-Desktop-Access-to-a-Domain-User](Remote-Desktop-Access-to-a-Domain-User.md) | Granting RDP to a domain user |
| [Ngrok-A-Local-Development-Tool](Ngrok-A-Local-Development-Tool.md) | Ngrok tunneling for local development |
| [RDP](RDP.md) | Remote Desktop Protocol (RDP) |
| [Remote-Desktop-Gateway](Remote-Desktop-Gateway.md) | Remote Desktop Gateway (HTTPS-wrapped RDP) |
| [OpenVPN](OpenVPN.md) | OpenVPN |
| [WireGuard](WireGuard.md) | WireGuard |
| [L2TP-IPsec](L2TP-IPsec.md) | L2TP/IPsec VPN |

## Practical Labs

> [!NOTE]
> **Hands-on labs**
> Guided labs for this topic are being consolidated under the course-wide **Practical-Labs** collection. Until then, stand up RRAS as an IKEv2 VPN server backed by NPS, connect a Windows client with certificate/EAP auth, then attempt a password spray against the endpoint and confirm NPS event `6273` and account lockout fire.
...
Microsoft's current guidance is to use **Always On VPN** (IKEv2-based, works with Windows 10/11 clients) rather than DirectAccess for anything newer than legacy Windows 7/8 estates.

Install the role and inspect it:
...
Practical guidance: prefer **IKEv2** for native Windows/mobile clients and **SSTP** as the firewall-friendly fallback where UDP is blocked outbound. Disable PPTP, and avoid L2TP/IPsec with pre-shared keys.

## VPN Server Configuration (RRAS)
...
- **Point RRAS at NPS** for RADIUS-based authentication/authorization instead of local Windows auth, so all remote-access policy lives centrally (see the Network Policy Server section below).

### What a Working Deployment Looks Like

Success, described (not fabricated as literal console output):
...
## Remote Desktop Services (RDS) / RDP Remote Access
...
RDP listens on **TCP/UDP 3389** by default. If publishing RDP externally, prefer routing it through an **RD Gateway** (wraps RDP in HTTPS/443) or a VPN rather than exposing 3389 directly to the internet.

## VPN and RRAS Logging and Monitoring

- **RRAS event/verbose tracing**: `netsh ras set tracing * enabled` writes detailed `.log` files under `%windir%\tracing` for troubleshooting connection failures. Disable again after diagnosis (`netsh ras set tracing * disabled`) — it is verbose and not meant to run permanently. # untested
...
NPS (installed with the **Network Policy and Access Services** role) is Microsoft's RADIUS server/proxy implementation (RFC 2865/2866), and is the standard way to centralize authentication, authorization, and accounting for VPN, RDS, and 802.1X wired/wireless access.
...
**MFA integration (concept):** the **NPS Extension for Azure MFA** installs on the NPS server and intercepts RADIUS authentication to add a cloud MFA challenge (push/call/OTP) before NPS grants access — useful for adding MFA to VPN or RD Gateway logons without changing the RADIUS client. Installation/registration steps are environment-specific (Azure AD app registration + extension MSI); verify current steps against Microsoft's own guide before deploying. # untested
...
- [Windows-Server](../Windows-Server-Management/Windows-Server.md) — backbone role/feature model RRAS and NPS install into
- [Active-Directory-Domain-Services](../Active-Directory-Domain-Services-AD-DS/Active-Directory-Domain-Services.md) — domain identity backing RADIUS/NPS auth and certificate-based EAP
- [Windows-Firewall-and-AV-Commands](../Windows-Commands/Windows-Firewall-and-AV-Commands.md) — opening/inspecting the firewall rules RDP/VPN/RADIUS depend on
- [Remote-Desktop-Access-to-a-Domain-User](Remote-Desktop-Access-to-a-Domain-User.md) — granting a domain user rights to RDP in
- [Port-Forwarding](../Proxy-Server-Administration/Port-Forwarding.md) — exposing internal services through NAT, contrast with proper VPN/Gateway access
- [Group-Policy(GPO)](../Group-Policy-Objects-GPO/Group-Policy(GPO).md) — centrally enforcing NLA, lockout, and remote-access policy at scale
- [Windows-Event-Logs](../Windows-Operating-System-Administration/Windows-Event-Logs.md) — general Windows logging/detection reference
- [Enterprise Windows Infrastructure Security](../Readme.md) — course MOC
